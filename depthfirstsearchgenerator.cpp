...
namespace maze
{
	virtual std::shared_ptr<maze::Maze> maze::DepthFirstSearchGenerator::make_maze() override
	{
    	...
    	*/
    	...
    	std::shared_ptr<maze::Maze> maze(new SquareMaze(height, width));
    	...
    	while(number_of_visited_cells < number_of_total_cells)
    	{
        	...
        	if(this.get_y_position() != 0)
        	{
            	maze::Cell * cell_above = maze->get_cell(this.get_x_position(), 
            		this.get_y_position() - 1);
        	...
        	/* One below */
       		maze::Cell * cell_below = maze->get_cell(this.get_x_position(), 
            	this.get_y_position() + 1);
        	if(cell_below != NULL)
        	...
        	if(this.get_x_position() != 0)
        	{
           		maze::Cell * cell_left = maze->get_cell(this.get_x_position() - 1, 
            		this.get_y_position());
            	if(cell_left != NULL)
        	...
        	/* One right */
        	maze::Cell * cell_right = maze->get_cell(this.get_x_position() + 1, 
            	this.get_y_position());
        	if(cell_right != NULL)
